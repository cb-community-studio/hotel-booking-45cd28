...
import hoteldetailsFakerFactory from "./hoteldetailsFakerFactory";
...
const HoteldetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = hoteldetailsFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("hoteldetails").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(HoteldetailsFakerDialogComponent);
