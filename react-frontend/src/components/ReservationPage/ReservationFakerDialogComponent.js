...
import reservationFakerFactory from "./reservationFakerFactory";
...
const ReservationFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = reservationFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("reservation").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(ReservationFakerDialogComponent);
